...
pip, npm, et des binaires vérifiés par checksum, derrière une seule
commande unifiée, sur macOS, Linux et Windows.
...
dans l'ordre — **tap → pip → npm** — et installe via le premier qui
signale que le paquet existe. Force un backend précis avec un préfixe :

| Commande | Résout vers |
|---|---|
| `poly install ripgrep` | première correspondance : tap → pip → npm |
...
l'entrée qui correspond à la machine sur laquelle il tourne.
